...
export function check(req, res, next) {
    ...
    if (!token)
        return res.status(401).json({ title: "user unauthorized", messgae: "first login!" })
    try {
...
export function checkManager(req, res, next) {
    ...
    try {
        ...
        if (result.role == "admin")
            next()
        return res.status(403).json({ title: "user unauthorized", messgae: "you dont have permision " })
    }
    catch (err) {
        return res.status(401).json({ title: "user unauthorized", messgae: err.message })
    }
